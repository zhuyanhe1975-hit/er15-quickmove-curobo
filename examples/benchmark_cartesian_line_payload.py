...
def main() -> None:
    parser = argparse.ArgumentParser(description="Benchmark ER15 QuickMove+TrueMove on a loaded rounded-door TCP path")
    ...
    print("Cartesian rounded-door payload benchmark")
    print(
        f"payload_kg={task.payload_kg:.1f} path_error={line_path.max_path_error_m * 1000.0:.3f} mm "
        f"width_y={task.width_y_m * 1000.0:.0f}mm height_z={task.height_z_m * 1000.0:.0f}mm "
        f"radius={task.corner_radius_m * 1000.0:.0f}mm"
    )
